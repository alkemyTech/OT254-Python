...
def procesar_moron(ti):
    ...
    y finalmente normaliza la tabla guardandola en un txt en la carpeta /files
    """
    ...
    #guardo el txt en la ruta de la carpeta /files escrita en el .env
    moron_df.to_csv(config('FILES')+"universidad_de_moron.txt", index=None)
...
def procesar_rio_cuarto(ti):
    ...
    #guardo el txt en la ruta de la carpeta /files escrita en el .env
    rio_df.to_csv(config('FILES')+"universidad_rio_cuarto.txt", index=None)
...
with DAG(
    'OT254_81',
    description="extrae, procesa y guarda en txt cada tabla procesada",
